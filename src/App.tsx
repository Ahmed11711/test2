    ...
    >
      ...
      <div className="hidden md:flex items-center gap-8 text-sm font-medium text-olive/80">
        ...
        <button
          ...
          className={cn(
            "hover:text-primary transition-colors",
            currentView === "story" && "text-primary font-bold",
          )}
        >
          قصتنا
        </button>
        <button
          ...
          className={cn(
            "hover:text-primary transition-colors",
            currentView === "blog" && "text-primary font-bold",
          )}
        >
          المدونة
        </button>
        <button
          ...
          className={cn(
            "hover:text-primary transition-colors",
            currentView === "contact" && "text-primary font-bold",
          )}
        >
          تواصل معنا
        </button>
      ...
      >
        ...
        >
          ...
          <p className="text-lg md:text-xl text-olive/70 leading-relaxed max-w-2xl mx-auto mb-10">
            أجب عن عدة أسئلة بسيطة وسنحدد لك التركيبة الأنسب لجسمك واحتياجاتك
            العضوية، عبر نظام ذكاء اصطناعي تفاعلي صُمم لراحتك.
          </p>
        </motion.div>
...
const QuizPortal = ({
  ...
  const calculateResult = (finalAnswers: boolean[]) => {
    ...
    if (cat === "sleep") {
      if (finalAnswers[0] && finalAnswers[1])
        finalProduct = PRODUCTS.find((p) => p.id === "peace-of-mind")!;
      else if (finalAnswers[2] && finalAnswers[3])
        finalProduct = PRODUCTS.find((p) => p.id === "stomach-balance")!;
      else if (finalAnswers[0] && !finalAnswers[2])
        finalProduct = PRODUCTS.find((p) => p.id === "clear-mind")!;
      else finalProduct = PRODUCTS.find((p) => p.id === "peace-of-mind")!;
    } else if (cat === "colon") {
      if (finalAnswers[0] && finalAnswers[1])
        finalProduct = PRODUCTS.find((p) => p.id === "abdominal-comfort")!;
      else if (finalAnswers[2])
        finalProduct = PRODUCTS.find((p) => p.id === "clear-mind")!;
      else if (finalAnswers[3])
        finalProduct = PRODUCTS.find((p) => p.id === "stomach-balance")!;
      else finalProduct = PRODUCTS.find((p) => p.id === "abdominal-comfort")!;
    } else if (cat === "stress") {
      if (finalAnswers[0] && finalAnswers[3])
        finalProduct = PRODUCTS.find((p) => p.id === "peace-of-mind")!;
      else if (finalAnswers[1] && finalAnswers[2])
        finalProduct = PRODUCTS.find((p) => p.id === "head-stillness")!;
      else if (finalAnswers[0])
        finalProduct = PRODUCTS.find((p) => p.id === "clear-mind")!;
      ...
      if (finalAnswers[0] && finalAnswers[1])
        finalProduct = PRODUCTS.find((p) => p.id === "body-shield")!;
      else if (finalAnswers[2])
        finalProduct = PRODUCTS.find((p) => p.id === "peace-of-mind")!;
    ...
    >
      ...
      {step === 1 && (
        <div className="max-w-4xl w-full">
          ...
          >
            <h2 className="text-4xl font-bold font-display text-olive mb-4">
              ما الذي تود تحسينه اليوم؟
            </h2>
            <p className="text-olive/60">
              اختر الفئة التي تعاني من أعراضها لنبدأ التشخيص
            </p>
          </motion.div>
            ...
            {QUIZ_DATA.map((cat, i) => (
              ...
              >
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                  ...
                  {cat.icon === "Wind" && (
                    <Wind className="w-8 h-8 text-sage" />
                  )}
                  {cat.icon === "Flame" && (
            ...
            >
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-gold">
                  ...
                  <span className="text-olive/60 text-sm mr-2">
                    (٢١٠ تقييم)
                  </span>
                </div>
    ...
    >
      ...
      >
        <div className="p-8 border-b border-olive/5 flex items-center justify-between">
          <h3 className="text-2xl font-bold font-display text-olive">
            سلة المشتريات
          </h3>
          <button
            onClick={onClose}
            className="p-2 hover:bg-olive/5 rounded-full transition-colors text-olive/40 hover:text-olive"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
            ...
            cart.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-olive truncate">
                    {item.arabicName}
                  </h4>
                  <p className="text-primary font-bold">
                    {item.priceNumber} ر.س
                  </p>
                  <div className="flex items-center gap-3 mt-2">
                    <button
                      onClick={() => onUpdateQty(item.id, -1)}
                      className="p-1 hover:bg-olive/10 rounded-lg transition-colors"
                    >
                      <Minus className="w-4 h-4" />
                    </button>
                    <span className="font-bold tabular-nums w-4 text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => onUpdateQty(item.id, 1)}
      ...
      <div className="max-w-4xl w-full">
              ...
              <div key={s} className="flex items-center">
                <div
                  className={cn(
                    ...
                    step === s
                      ? "bg-primary text-white transform scale-125"
                      : step > s
                        ? "bg-sage text-white"
                        : "bg-olive/5 text-olive/40",
                  )}
                >
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              <div className="space-y-8">
                <h2 className="text-3xl font-bold font-display text-olive">
                  عنوان الشحن
                </h2>
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <label className="text-sm font-bold text-olive/60">
                      الاسم الكامل
                    </label>
                    <input
                      type="text"
                      className="w-full bg-white border border-olive/10 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-sage"
                      placeholder="مثلاً: وسام قن"
                    />
                  </div>
                  <div className="grid gap-2">
                    <label className="text-sm font-bold text-olive/60">
                    ...
                    <input
                      type="tel"
                      className="w-full bg-white border border-olive/10 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-sage font-mono"
                      placeholder="+966 5..."
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    >
                      <div className="flex gap-4">
                        <div className="relative">
                          ...
                          />
                          <span className="absolute -top-2 -right-2 bg-sage text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold">
                            {item.quantity}
                          </span>
                        </div>
                        <div>
                      ...
                      <p className="font-bold text-primary">
                        {item.priceNumber * item.quantity} ر.س
                      </p>
                    </div>
                  ))}
          ...
          {step === 2 && (
            ...
            >
              <h2 className="text-4xl font-bold font-display text-olive text-center">
                طريقة الدفع
              </h2>
              <div className="grid gap-6">
                <div className="glass p-8 rounded-3xl border-2 border-primary ring-4 ring-primary/5 flex items-center gap-6 cursor-pointer">
                  ...
                  <div className="flex-1">
                    <h4 className="text-xl font-bold text-olive">
                      بطاقة مدى / أبل باي
                    </h4>
          ...
          {step === 3 && (
            ...
            >
              ...
              <h2 className="text-5xl font-bold font-display text-olive">
                تم استلام طلبك بنجاح!
              </h2>
              <p className="text-xl text-olive/60 max-w-lg mx-auto leading-relaxed">
                شكراً لثقتك بـ "ترياق". سنقوم بتوصيل خلطتك العشبية المخصصة إليك
                في غضون ٢-٤ أيام عمل.
              </p>
              <button
                onClick={onSuccess}
    ...
    >
      ...
      <div className="p-8 space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-sm font-bold text-sage bg-sage/10 px-3 py-1 rounded-full">
            {product.name}
          </span>
        ...
        <p className="text-olive/60 line-clamp-2 text-sm leading-relaxed">
          {product.description}
        </p>
    ...
    switch (view) {
        ...
        return (
          ...
          >
            <div className="space-y-6">
              <span className="text-sage font-bold tracking-widest uppercase text-sm">
                تراثنا ورؤيتنا
              </span>
              <h1 className="text-5xl md:text-7xl font-bold font-display text-olive">
                قصة ترياق
              </h1>
              <p className="text-xl text-olive/70 leading-relaxed max-w-3xl mx-auto">
                بدأ "ترياق" من شغف عميق بالحكمة العربية القديمة في العلاج
                بالأعشاب، مدمجة بأحدث التقنيات الرقمية لنقدم لك حلاً مخصصاً
                يتناسب مع احتياجات جسمك الفريدة.
              </p>
            </div>

            <div className="relative grid md:grid-cols-2 gap-12 text-right p-12 rounded-[4rem] overflow-hidden bg-[#fffefb]">
              <img
                src="/src/assets/images/regenerated_image_1779135519672.jpg"
              ...
              <div className="glass p-10 rounded-[3rem] space-y-6 border-white/40 shadow-2xl">
                <h3 className="text-3xl font-bold text-olive">
                  أصالة المكونات
                </h3>
                <p className="text-olive/60 leading-relaxed text-lg">
                  نحن نجوب الجبال والوديان لنبحث عن أنقى الأعشاب العضوية، ونحرص
                  على أن تُقطف في أوج جودتها لتمنحك ترياقاً حقيقياً.
                </p>
              </div>
              <div className="glass p-10 rounded-[3rem] space-y-6 border-white/40 shadow-2xl">
                <h3 className="text-3xl font-bold text-olive">
                ...
                <p className="text-olive/60 leading-relaxed text-lg">
                  عبر محركنا الذكي، نقوم بتحليل أعراضك واحتياجاتك لنقترح لك
                  التركيبة التي تعيد التوازن لحياتك اليومية.
                </p>
              </div>
            </div>

            <img
              src="/src/assets/images/regenerated_image_1779135525409.jpg"
        ...
        return (
          ...
          >
            <div className="text-center space-y-4">
              ...
              <h2 className="text-4xl md:text-5xl font-bold font-display text-olive">
                مقالات للصحة والوعي
              </h2>
            </div>
                ...
                >
                  ...
                  <div className="p-8 space-y-4 text-right">
                    ...
                    <h3 className="text-xl font-bold text-olive">
                      كيف تبدأ يومك بوعي وطاقة عشبية؟
                    </h3>
                    <p className="text-olive/60 text-sm">
                      اكتشف أفضل الخلطات الصباحية التي تمنحك التركيز والنشاط
                      الطبيعي بدون كافيين...
                    </p>
        ...
        return (
          ...
          >
            <div className="glass rounded-[4rem] p-12 md:p-20 text-center space-y-12">
              <div className="space-y-4">
                <h1 className="text-4xl md:text-6xl font-bold font-display text-olive">
                  تواصل معنا
                </h1>
                <p className="text-olive/60 text-lg">
                  فريقنا جاهز للإجابة على جميع استفساراتك حول خلطاتنا.
                </p>
              </div>
              ...
              <div className="grid gap-6 text-right">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-olive/40 mr-4">
                      الاسم
                    </label>
                    <input
                      type="text"
                      className="w-full bg-white/50 border border-olive/10 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-sage"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-olive/40 mr-4">
                ...
                <button className="bg-primary text-white py-5 rounded-2xl text-xl font-bold shadow-xl hover:scale-105 active:scale-95 transition-all">
                  إرسال الرسالة
                </button>
              </div>
            </div>
      ...
      case "faq":
        return (
          ...
          >
            <div className="text-center space-y-4">
              <h1 className="text-4xl font-bold font-display text-olive">
                الأسئلة الشائعة
              </h1>
              <p className="text-olive/60">
                كل ما تحتاج لمعرفته حول منتجاتنا وخدماتنا
              </p>
            </div>
            <div className="space-y-4">
              {[
                {
                  q: "كيف أختار الخلطة المناسبة لي؟",
                  a: "يمكنك استخدام نظام التشخيص الذكي الخاص بنا (اختر خلطتك) للإجابة على بعض الأسئلة وسنقوم باقتراح الأنسب لك.",
                },
                {
                  q: "هل الأعشاب المستخدمة عضوية؟",
                  a: "نعم، جميع أعشابنا عضوية بنسبة ١٠٠٪ ومختارة بعناية من مصادر مستدامة.",
                },
                {
                  q: "كم يستغرق التوصيل؟",
                  a: "يستغرق التوصيل عادة من ٢ إلى ٤ أيام عمل لجميع مناطق المملكة.",
                },
                {
                  q: "هل توجد موانع استخدام؟",
                  a: "نوصي دائماً باستشارة طبيبك قبل البدء بأي برنامج عشبي، خاصة إذا كنت تعاني من حالات طبية خاصة أو تتناول أدوية.",
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className="glass p-8 rounded-3xl text-right space-y-4"
                >
        ...
        return (
          ...
          >
            ...
            <div className="glass p-12 rounded-[3rem] space-y-8 text-olive/70 leading-loose">
              <p>
                باستخدامك لموقع ترياق، فإنك توافق على الالتزام بالشروط والأحكام
                التالية:
              </p>
              <ul className="list-disc pr-6 space-y-4">
                <li>
                  المعلومات المقدمة في الموقع هي لأغراض تعليمية وتثقيفية فقط.
                </li>
                <li>نحن نحتفظ بالحق في تعديل الأسعار أو المنتجات في أي وقت.</li>
                <li>يجب أن تكون في السن القانونية لإجراء عمليات الشراء.</li>
                <li>
                  يتم التعامل مع جميع البيانات الشخصية وفقاً لسياسة الخصوصية.
                </li>
              </ul>
            </div>
          </motion.section>
        ...
        return (
          ...
          >
            <div className="bg-red-50 border-2 border-red-100 p-12 rounded-[3.5rem] space-y-8">
              <h1 className="text-4xl font-bold font-display text-red-900">
                إخلاء المسؤولية الطبي
              </h1>
              <div className="text-red-800/80 leading-loose text-lg space-y-6">
                <p>
                  منتجات ترياق هي مكملات عشبية طبيعية وليست أدوية كيميائية. لم
                  يتم تقييم هذه المنتجات من قبل الهيئات الصحية لتشخيص أو علاج أو
                  شفاء أو منع أي مرض.
                </p>
                <p>
                  يُرجى استشارة ممارس رعاية صحية مرخص قبل استخدام أي منتج عشبي،
                  خاصة إذا كنتِ حاملاً، أو مرضعة، أو تتناولين أدوية طبية، أو
                  لديك حالة صحية مشخصة.
                </p>
                <p>
                  ترياق لا يتحمل أي مسؤولية عن سوء استخدام المنتجات أو تجاهل
          ...
          >
            <div className="text-center space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl font-bold font-display text-olive">
                {QUIZ_DATA.find((c) => c.id === selectedCategory)?.title ||
                  "المنتجات"}
              </h1>
            ...
            {PRODUCTS.filter(
              ...
              <div className="text-center py-20 text-olive/40 space-y-4">
                ...
                <p className="text-xl font-bold">
                  عذراً، لا توجد منتجات حالياً في هذه الفئة
                </p>
                <button
        ...
        return (
          ...
          >
              ...
              <div className="space-y-10 text-right">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 text-gold">
                      ...
                      <span className="text-olive/40 text-sm mr-2">
                    ...
                    <span className="text-sage font-bold tracking-widest uppercase text-sm border-b-2 border-sage/20 pb-1">
                      {
                        QUIZ_DATA.find((c) => c.id === selectedProduct.category)
                          ?.title
                      }
                    </span>
                  </div>
                  <h1 className="text-5xl md:text-6xl font-bold font-display text-olive">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="glass p-8 rounded-3xl space-y-4">
                    <h4 className="text-lg font-bold text-sage">
                      المكونات العضوية
                    </h4>
            ...
            <div className="mt-32 space-y-12">
              <h2 className="text-3xl font-bold font-display text-olive text-center">
                أهم الأسئلة حول {selectedProduct.arabicName}
              </h2>
              <div className="grid md:grid-cols-2 gap-6">
                {[
                  {
                    q: "كيفية التحضير؟",
                    a: "يُنصح بوضع ملعقة منه في ماء حار لمدة ١٠ دقائق قبل الشرب.",
                  },
                  {
                    q: "كم مرة يُستخدم؟",
                    a: "يُفضل استخدامه مرة أو مرتين يومياً للحصول على أفضل توازن.",
                  },
                ].map((faq, i) => (
            ...
            <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
              ...
              <div className="relative text-center text-white space-y-8 px-6 max-w-4xl">
                <h2 className="text-5xl md:text-7xl font-bold font-display leading-tight">
                  نعيد تعريف علاقتك <br />{" "}
                  <span className="italic text-sage">بالطبيعة</span>
                </h2>
                <p className="text-xl text-white/80 leading-relaxed">
                  نؤمن أن الطبيعة تمتلك الإجابات لكل تساؤلات أجسامنا. نحن هنا
                  فقط لنجمع لك أصفاها.
                </p>
                <button
    ...
    <div className="antialiased font-sans selection:bg-sage/30">
      ...
      <footer className="py-20 px-6 border-t border-olive/5 bg-olive/5">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-12">
          <div className="space-y-6">
            <Logo />
            <p className="text-olive/60 text-sm leading-relaxed">
              أول منصة ذكية للعلاجات العشبية المخصصة في العالم العربي. نجمع بين
              الحكمة القديمة والتقنيات الحديثة باسم "ترياق".
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-olive/60">
              <li>
                <button
                  onClick={() => handleCategoryView("sleep")}
              ...
              <li>
                <button
                  onClick={() => handleCategoryView("colon")}
                  className="hover:text-primary transition-colors"
                >
                  علاجات القولون
                </button>
              ...
              <li>
                <button
                  onClick={() => handleCategoryView("immunity")}
                  className="hover:text-primary transition-colors"
                >
                  تقوية المناعة
                </button>
              ...
              <li>
                <button
                  onClick={() => handleCategoryView("headache")}
                  className="hover:text-primary transition-colors"
                >
                  منتجات الصداع
                </button>
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <button className="absolute left-2 top-2 bg-sage text-white px-4 py-1 rounded-lg text-sm font-bold">
                انضم
              </button>
            </div>
          </div>
